...
const core = ls => ls.map(({ h6, qty, price, name }) => ({ h6, qty, price, name })); // parser rows minus the per-line cur/h10 added 2026-09-21
...
t('GPT-6 Pro 2026-09-21: ¥ follows the document currency, KRW/JPY lines keep their own currency, 10-digit codes survive, names split rows', () => {
  let P = BizCalc.parseSheet('통화 JPY\n[620453] 스커트 · 수량 2개 · 단가 ¥1000');
  assert.deepEqual([P.cur, P.lines[0].cur, P.lines[0].price], ['JPY', 'JPY', 1000]);
  P = BizCalc.parseSheet('[620453] 스커트 · 수량 2개 · 단가 ¥1000');
  assert.deepEqual([P.cur, P.lines[0].cur], ['CNY', 'CNY']); // no code anywhere → Chinese forwarder default
  P = BizCalc.parseSheet('[950730] 릴 · 수량 1개 · 단가 100 USD\n[950730] 케이스 · 수량 1개 · 단가 10000 원');
  assert.deepEqual(P.lines.map(l => [l.cur, l.price]), [['USD', 100], ['KRW', 10000]]); assert.equal(P.cur, 'USD'); assert.equal(P.mixed, false);
  P = BizCalc.parseSheet('HS 6204.53.0000 · 수량 3개 · 단가 5 USD');
  assert.deepEqual([P.lines[0].h6, P.lines[0].h10], ['620453', '6204530000']);
  P = BizCalc.parseSheet('[950730] 빨간 릴 · 수량 10개 · 단가 100 CNY\n[950730] 파란 릴 · 수량 20개 · 단가 100 CNY\n[950730] 빨간 릴 · 수량 5개 · 단가 100 CNY');
  assert.deepEqual(P.lines.map(l => [l.name, l.qty]), [['빨간 릴', 15], ['파란 릴', 20]]); // same 품명 merges, a different one does not
});

t('GPT-6 Pro 2026-09-21: books are VAT-exempt, unclassified priced lines are reported, 세관장확인 groups are never auto-resolved', () => {
  const book = HS.codes.find(c => c.c.startsWith('4901'));
  const r = BizCalc.compute({ fx, cur: 'USD', cols: HS.cols, origin: 'US', co: false, freight: 0, freightCur: 'KRW', lines: [{ entry: book, qty: 1, price: 1000 }, { entry: null, qty: 1, price: 10000 }] });
  assert.equal(r.lines[0].vat, 0); assert.equal(r.lines[0].vatRate, 0); assert.equal(r.vatExemptLines, 1);
  assert.deepEqual([r.pending, r.pendingKrw], [1, Math.floor(10000 * fx.USD)]);
  const reelNoVat = BizCalc.compute({ fx, cur: 'USD', cols: HS.cols, origin: 'US', co: false, freight: 0, freightCur: 'KRW', lines: [{ entry: reel, qty: 1, price: 100 }] });
  assert.ok(reelNoVat.lines[0].vat > 0);
  const withQ = {}; for (const c of HS.codes) if (c.q) (withQ[c.c.slice(0, 6)] = withQ[c.c.slice(0, 6)] || []).push(c);
  const multi = Object.entries(withQ).find(([h6, g]) => g.length > 1 && HS.codes.filter(c => c.c.startsWith(h6)).length === g.length);
  if (multi) assert.equal(BizCalc.resolveH6(HS.codes, multi[0]).entry, null, 'a 세관장확인 group with several rows must be picked by hand');
});
...
t('parseSheet: QuickStar 신청서조회 text → merged lines, currency, declared total, shipping', () => {
  ...
  assert.deepEqual(core(P.lines), [{ h6: '620453', qty: 235, price: 10.43, name: '치마' }, { h6: '871499', qty: 350, price: 3.82, name: '자전거보조용품' }, { h6: '871499', qty: 150, price: 4.83, name: '자전거보조용품' },
...
t('parseSheet: other forwarders — table with 개/$·¥ symbols, HS코드: 10-digit, dotted HS with pcs/Unit price, and a bare price is not an HS code', () => {
  ...
  assert.deepEqual(core(P.lines), [{ h6: '620443', qty: 100, price: 12.5, name: '원피스' }, { h6: '961511', qty: 2000, price: 0.85, name: '헤어핀' }]); // table row: name before the code
  assert.deepEqual([P.cur, P.totalKrw, P.exclKrw, P.co, P.mixed], ['USD', 320000, 70000, true, false]);
  P = BizCalc.parseSheet('품명: 낚시릴\nHS코드: 9507300000\n수량: 40\n가격: 28.5 USD\n\n품명: 낚싯대\nHS코드 950710\n수량 25\n가격 14 USD\n총 결제금액 150,000 KRW');
  assert.deepEqual(core(P.lines), [{ h6: '950730', qty: 40, price: 28.5, name: '낚시릴' }, { h6: '950710', qty: 25, price: 14, name: '낚싯대' }]); assert.equal(P.totalKrw, 150000); // 품명: label above
  P = BizCalc.parseSheet('Item 1 - HS 8714.99.00 - 350 pcs - Unit price 3.82 CNY\nItem 2 - HS 6307.10 - 3000 pcs - Unit price ¥0.14\n국제운임 210,000 원\n원산지증명서 발급 30,000원\n원산지작업 40,000원');
  assert.deepEqual(core(P.lines), [{ h6: '871499', qty: 350, price: 3.82, name: 'Item 1' }, { h6: '630710', qty: 3000, price: 0.14, name: 'Item 2' }]); // no Hangul anywhere → whatever precedes, never the qty/price cells
